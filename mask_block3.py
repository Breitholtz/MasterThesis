...
XMAX=256 
YMAX=341 
blocked=0 # how much of the image has on average been blocked out?
...
## act is the gradients and img is the image data
 ...
 # print normgrad[indices[0,10000,0],indices[0,10000,1]]
  ...
  P=(2*padding+1)*(2*padding+1)  
  Amount=L/float(P)
  #MEAN=[1,1,1]
  # make the important pixels be greyed out
  for i in range(L-1,L-L2-1,-1):
    X=indices[0,i,0]
    Y=indices[0,i,1]
    ...
    for i1 in range(-xminus,xplus):
      for i2 in range(-yminus,yplus):
          ...
          bin_mat[X+i1,Y+i2]=1
  blocked=blocked+np.sum(bin_mat)/float(256*341)
  #print blocked
  #plt.imshow(newimg)
  #plt.show() 
