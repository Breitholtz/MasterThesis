...
# intrinsic camera parameters (for 7Scenes dataset) Necessary??
u0=320
v0=240
ax=585
ay=585
 ...
 # get pose and depth, works for 7Scenes dataset; probably really slow..
  if not train:
      if batch_idx<1000:
        ...
        depth=imageio.imread(osp.join(seq_dir[1],'frame-{:06d}.depth.png'.format(batch_idx)))
  else:
      if batch_idx<1000:
        poses=np.loadtxt(osp.join(seq_dir[0],'frame-{:06d}.pose.txt'.format(batch_idx)))
        depth=imageio.imread(osp.join(seq_dir[0],'frame-{:06d}.depth.png'.format(batch_idx)))
        ...
        depth=imageio.imread(osp.join(seq_dir[2],'frame-{:06d}.depth.png'.format(batch_idx)))
      else:
        poses=np.loadtxt(osp.join(seq_dir[3],'frame-{:06d}.pose.txt'.format(batch_idx-3000)))
        depth=imageio.imread(osp.join(seq_dir[3],'frame-{:06d}.depth.png'.format(batch_idx)))
...
## act is the gradients and img is the image data
 ...
 ## presence is defined if an important point is within a certain pixel distance
  ...
  for i in range(L-1,L-1-L2,-1):
    ...
    Z= depth[indices_prev[0][i][0]][indices_prev[0][i][1]] 
    if Z<=0:
...
# when all the projected points are known we just compare them to the top X% of gradient points in frame 2 and save the overlap
  ...
  for i in range(L-1,L-1-L2,-1):
    ...
    for j in range(L-1,L-1-L2,-1):
     X1=indices_prev[0,j,0]
     Y1=indices_prev[0,j,1]
     if X1==X2 and Y1==Y2:
            #print X1
            #print X2
            #print indices2[0,j]
