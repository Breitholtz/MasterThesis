...
parser = argparse.ArgumentParser(description='Evaluation script for PoseNet and'
                                             'MapNet variants')
parser.add_argument('--dataset', type=str, choices=('7Scenes', 'RobotCar','blockdata'),
                    help='Dataset')
...
parser.add_argument('--percent',type=int) # to know how much is masked
...
if (args.model.find('mapnet') >= 0) or args.pose_graph:
  ...
  L = len(data_set)
elif args.dataset == 'blockdata':
  from dataset_loaders.block_data import blockdata
  data_set = blockdata(**kwargs)
  L = len(data_set)
