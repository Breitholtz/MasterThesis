...
## act is the gradients and img is the image data
 ...
 ### with the mean colour of the dataset and save or use the new image somewhere
  ...
  # we want to block out the top percentage by using X-by-X squares centered on the most important points
  L=img.shape[0]*img.shape[1]
  percent_amount=np.floor((L*percentage)/100)
  L2=int(L-percent_amount)
  padding=4 # amount of pixels around each point that gets greyed out
   # search through gradient image and grey out blocks in the real image if the gradient value is above the threshold
  newimg=img
  ...
  count=0
  out_filename = osp.join(args.output_dir, 'frame-{:d}.color.png'. format( batch_idx))
  #print L
  #print L2
  #BLK=[0,0,0]
  for i in range(L-1,L2,-1):
      X=indices[0,i,0]
      Y=indices[0,i,1]
     # if newimg[X,Y].astype(int)[0]==BLK[0]:#MEAN[0]: Can be used to check if already greyed out 
      #    continue
      if X+padding>=newimg.shape[0] or X-padding<0 or Y+padding>=newimg.shape[1] or Y-padding<0:
          # this is a case where we are less than the patch size from the edge of the image
          # we probably want to still block out that which we can from the image
          continue 
          xminus=padding
          xplus=padding
          yminus=padding
          yplus=padding
          if X-padding<0: 
           xminus=padding-abs(X-padding)
           #print 'x-:{}'.format(xminus)
          if X+padding>=newimg.shape[0]:
           xplus=abs(newimg.shape[0]-X)#-padding)
           #print 'x+:{}'.format(xplus)
           #print X
          if Y-padding<0:
           yminus=abs(Y-padding)
           #print 'y-:{}'.format(yminus)
          if newimg.shape[1]<=Y+padding:
           yplus=abs(newimg.shape[1]-Y)#-padding)
          # print 'y+:{}'.format(yplus) 
      
          for j in range(-xminus,xplus):
            for k in range(-yminus,yplus):
                 newimg[X+j,Y+k]=MEAN
      else:
        for i1 in range(-padding,padding):
          for i2 in range(-padding,padding):
             newimg[X+i1,Y+i2]=MEAN
