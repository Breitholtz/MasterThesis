...
L=256*341
L2=np.floor(L*percentage/100.0)       
# area of masking squares
padding=7
Area=(2*padding+1)*(2*padding+1)
...
## act is the gradients and img is the image data
 ...
 # print normgrad[indices[0,10000,0],indices[0,10000,1]]
  ...
  #MEAN=[1,1,1]
 #  P= L*percentage/float(100)
  # make the important pixels be greyed out
  for i in range(L-1,int(L-L2-1),-1):
    X=indices[0,i,0]
    Y=indices[0,i,1]
    if (L-1-i)*Area>L2:
     # check if we have greyed out enough of the image yet  
       numpix=np.sum(bin_mat)
       #for x in range(0,XMAX-1):
#	for y in range(0,YMAX-1): 
#	 if bin_mat[x][y]==1: #newimg[x][y][0]==MEAN[0] and newimg[x][y][1]==MEAN[1] and newimg[x][y][2]==MEAN[2]:
#	  numpix+=1

       if numpix>L2:
	break
	# handle points closer than the padding distance to the edge of the image
    xminus=padding
    xplus=padding
    yminus=padding
    yplus=padding
    if X-padding<0:
	 xminus=padding+padding-X
    if X+padding>=XMAX:
	 xplus=XMAX-X
    if Y-padding<0:
	 yminus=padding+padding-Y
    if Y+padding>=YMAX:
	 yplus=YMAX-Y  
	# mask a square around the point
    for i1 in range(-xminus,xplus):
      for i2 in range(-yminus,yplus):
          newimg[X+i1,Y+i2]=MEAN#MEAN
          bin_mat[X+i1,Y+i2]=1
