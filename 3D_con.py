...
#  threshold=0 # distance from projected point which the top X% point may lie while counting as corresponding
  ...
  L3=len(dpoints)
  if L3!=0:
    match_percentage=match_percentage+matches/float(L3)
...
if sampling==True:
 ...
 f.write('{:d}%:'.format(args.percent)+str(match_percentage/float(batch_idx-frame_count))+'\n')
f.close()
sys.exit(-1)
